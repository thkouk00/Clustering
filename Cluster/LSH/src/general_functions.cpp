...
void search_neighbors(Cluster** cluster, HashTable **hashTables,std::vector<std::string> &id,std::vector<std::vector<double>> &queryset,int &L,int &k,int &w, int &num_of_buckets, double &Radius,bool Euclidean)
{
	int counter=0;
	int tmpfi;
	...
	for (id_iter=id.begin(), it=queryset.begin();it!=queryset.end();it++, id_iter++)
    {
		...
		Range_search(cluster,hashTables,g,query,fi,L,k,Radius,Euclidean,TrueDist);
