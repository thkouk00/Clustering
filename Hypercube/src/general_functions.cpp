...
void search_neighbors(HashTable *cube,std::vector<std::string> &id,std::vector<std::vector<double>> &queryset,std::map<int,bool> &mymap,int &M, int &probes,int &k,int &w, int &num_of_buckets, double &Radius,bool &Euclidean)
{
	int position;
	...
	for (id_iter=id.begin(), it=queryset.begin();it!=queryset.end();it++, id_iter++)
    {
		...
		Range_search(cube,g,query,TmpPos,M,probes,k,Radius,Euclidean,TrueDist);
...
int find_hashFunction(std::vector<int> &g, std::vector<double> &query, std::map<int,bool> &mymap, int &k, int &w, int &num_of_buckets, int &position, bool Euclidean)
{
	int h;
	double t;
	std::vector<double> v;
